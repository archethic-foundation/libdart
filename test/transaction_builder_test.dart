  ...
  group('Transaction builder', () {
    ...
    group('setContent', () {
      test('should insert the content into the transaction data', () {
        var tx =
            new TransactionBuilder("transfer").setContent("my super content");
        expect(utf8.decode(tx.data!.content!), "my super content");
      });
    });
    group('setSecret', () {
      test('should insert the secret into the transaction data', () {
        var tx = new TransactionBuilder("transfer").setSecret(
    ...
    group('addUCOTransfer', () {
      test('should add an uco transfer to the transaction data', () {
        var tx = new TransactionBuilder("transfer").addUCOTransfer(
            "00b1d3750edb9381c96b1a975a55b5b4e4fb37bfab104c10b0b6c9a00433ec4646",
            10.03);
    ...
    group('addNFTTransfer', () {
      test('should add an nft transfer to the transaction data', () {
        var tx = new TransactionBuilder("transfer").addNFTTransfer(
            "00b1d3750edb9381c96b1a975a55b5b4e4fb37bfab104c10b0b6c9a00433ec4646",
            10.03,
            "00b1d3750edb9381c96b1a975a55b5b4e4fb37bfab104c10b0b6c9a00433ec4646");
    ...
    group('previousSignaturePayload', () {
      test('should generate binary encoding of the transaction before signing',
        ...
        KeyPair nextKeypair = crypto.deriveKeyPair("seed", 1);
        Uint8List address = crypto.hash(nextKeypair.publicKey);

        tx.address = address;
        tx.previousPublicKey = keypair.publicKey;
        tx.timestamp = new DateTime.now().millisecondsSinceEpoch;
        Uint8List payload = tx.previousSignaturePayload();
    ...
    group('originSignaturePayload', () {
      test('should generate binary encoding of the transaction before signing',
        ...
        KeyPair transactionKeyPair = crypto.deriveKeyPair("seed", 0);
        Uint8List previousSig = crypto.sign(
            tx.previousSignaturePayload(), transactionKeyPair.privateKey);

        Uint8List payload = tx.originSignaturePayload();
    ...
    group('originSign', () {
      test('should sign the transaction with a origin private key', () {
        KeyPair originKeypair = crypto.deriveKeyPair("origin_seed", 0);
    ...
    group('toJSON', () {
      test('should return a JSON from the transaction', () {
        KeyPair originKeypair = crypto.deriveKeyPair("origin_seed", 0);
        KeyPair transactionKeyPair = crypto.deriveKeyPair("seed", 0);
        KeyPair nextTransactionKeyPair = crypto.deriveKeyPair("seed", 1);
