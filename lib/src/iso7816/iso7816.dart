...
class ISO7816_INS {
  ...
  static const int READ_BINARY_EXT =
      0xB1; // read instruction when file chunk offset is larger than 32767 bytes
...
class ISO97816_SelectFileP1 {
  static const int byID =
      0x00; // Select MF, DF or EF (data field=identifier or empty)
  static const int byChildDFID =
      0x01; // Select child DF (data field=DF identifier)
  static const int byEFID =
      0x02; // Select EF under current DF (data field=EF identifier)
  static const int parentDF = 0x03; // Select parent DF of the current DF
  static const int byDFName =
      0x04; // Direct selection by DF name (data field=DF name)
  static const int byPathFromMF =
      0x08; // Select from MF (data field=path without the identifier of the MF)
  static const int byPath =
      0x09; // Select from current DF (data field=path without the identifier of the current DF)
}
